...
INIT_FUNC(mod_fastcgi_reproxy_init) {
    plugin_data *p;
...
FREE_FUNC(mod_fastcgi_reproxy_free) {
    plugin_data *p = p_d;
...
SETDEFAULTS_FUNC(mod_fastcgi_reproxy_set_defaults) {
    plugin_data *p = p_d;
    data_unset *du;
    size_t i = 0;
 ...
 * */
SUBREQUEST_FUNC(mod_fastcgi_reproxy_handle_subrequest) {
    plugin_data *p = p_d;
...
static handler_t fcgi_handle_fdevent(void *s, void *ctx, int revents) {
    ...
    if (revents & FDEVENT_OUT) {
        if (hctx->state == FCGI_STATE_CONNECT_DELAYED ||
             ...
             */
            return mod_fastcgi_reproxy_handle_subrequest(srv, con, p);
        } else {
            log_error_write(srv, __FILE__, __LINE__, "sd",
    ...
    if (uri_path_handler) {
        if (host->check_local == 0) {
            ...
            if (con->conf.log_request_handling) {
                log_error_write(srv, __FILE__, __LINE__, "s",
                "handling it in mod_fastcgi_reproxy");
            }
        ...
        if (con->conf.log_request_handling) {
            log_error_write(srv, __FILE__, __LINE__, "s", "handling it in mod_fastcgi_reproxy");
        }
    }
...
JOBLIST_FUNC(mod_fastcgi_reproxy_handle_joblist) {
    plugin_data *p = p_d;
    handler_ctx *hctx = con->plugin_ctx[p->id];
...
TRIGGER_FUNC(mod_fastcgi_reproxy_handle_trigger) {
    plugin_data *p = p_d;
    size_t i, j, n;
...
int mod_fastcgi_reproxy_plugin_init(plugin *p) {
    ...
    p->set_defaults = mod_fastcgi_reproxy_set_defaults;
    ...
    p->handle_subrequest       = mod_fastcgi_reproxy_handle_subrequest;
    p->handle_joblist          = mod_fastcgi_reproxy_handle_joblist;
    p->handle_trigger          = mod_fastcgi_reproxy_handle_trigger;
